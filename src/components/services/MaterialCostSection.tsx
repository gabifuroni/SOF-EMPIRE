...
const MaterialCostSection = ({ materialCosts, materials, onMaterialCostsChange }: MaterialCostSectionProps) => {
  // New material added at TOP (index 0), pushing older ones down
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <style>{`
        ...
        .mat-add-btn:hover { border-color: #3a3a4a !important; color: #f0f0f8 !important; }
      `}</style>
      ...
      {/* Material rows — newest at top, oldest at bottom */}
      {materialCosts.map((mc, index) => (
        <div key={index} style={{ background: '#1c1c26', border: '1px solid #2a2a38', borderRadius: 10, padding: '14px 16px', marginBottom: 8, display: 'grid', gridTemplateColumns: '2fr 1fr 1fr auto', gap: 12, alignItems: 'end' }}>
          <div>
            ...
            <select className="mat-select" value={mc.materialId} onChange={e => updateMaterial(index, 'materialId', e.target.value)} style={inputStyle}>
          ...
          <div>
            ...
            <input className="mat-input" type="number" step="0.01" value={mc.quantity} onChange={e => updateMaterial(index, 'quantity', parseFloat(e.target.value) || 0)} style={inputStyle} />
          </div>
          <div>
            <label style={labelStyle}>Custo Total</label>
            <input style={{ ...inputStyle, background: '#13131a', color: '#00c896', fontWeight: 600, cursor: 'not-allowed' }} value={`R$ ${mc.cost.toFixed(2)}`} readOnly />
          </div>
          <button type="button" className="mat-del-btn" onClick={() => removeMaterial(index)} style={{ background: 'rgba(255,77,106,0.08)', border: '1px solid rgba(255,77,106,0.2)', borderRadius: 8, padding: '9px 10px', cursor: 'pointer', color: '#ff4d6a', display: 'flex', alignItems: 'center', transition: 'all 0.15s', alignSelf: 'flex-end' }}>
            <Trash2 size={15} />
          </button>
        </div>
      ))}
      ...
      {materialCosts.length === 0 && (
        <div style={{ border: '1px dashed #2a2a38', borderRadius: 10, padding: '16px', textAlign: 'center', color: '#606078', fontSize: 12 }}>
