...
const AddServiceModal = ({ show, service, materials, onClose, onSave }: AddServiceModalProps) => {
  ...
  useEffect(() => {
    if (service) {
      setFormData({
        ...
        cardTaxRate: service.cardTaxRate,
        serviceTaxRate: service.serviceTaxRate,
  ...
  const calculateCosts = () => {
    ...
    const cardTaxCost = (formData.salePrice * formData.cardTaxRate) / 100;
    const serviceTaxCost = (formData.salePrice * formData.serviceTaxRate) / 100;
    ...
    return { totalCost, grossProfit, profitMargin };
  };
    ...
    <Dialog open={show} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-symbol-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <TaxRatesSection
            taxRates={{
              cardTaxRate: formData.cardTaxRate,
              serviceTaxRate: formData.serviceTaxRate,
              commissionRate: formData.commissionRate
            }}
            onTaxRatesChange={(rates) => setFormData({ ...formData, ...rates })}
          />
