  ...
  return (
    <header className="bg-symbol-white/80 backdrop-blur-sm border-b border-symbol-gray-200 px-4 sm:px-6 py-4 sticky top-0 z-30">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="hidden sm:block text-right">
            <p className="brand-body text-symbol-black text-sm sm:text-base">{user.name}</p>
            {user.salonName && (
              <p className="text-xs sm:text-sm text-symbol-gray-600">{user.salonName}</p>
            )}
          </div>
      ...
      <div className="sm:hidden mt-2 text-left">
        <p className="font-medium text-elite-charcoal-800 text-sm">{user.name}</p>
        {user.salonName && (
          <p className="text-xs text-elite-charcoal-600">{user.salonName}</p>
        )}
      </div>
    </header>
  );
